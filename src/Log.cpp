...
#undef XX
    ...
    std::string StdoutLogAppender::toYamlString() {
        ...
        //node["level"] = LogLevel::toString(m_level);
        if (m_hasFormatter && m_log_formatter) {
...
#undef XX
        ...
        for (auto &i: format_vec) {
            if (std::get<2>(i) == 0) {
                ...
                if (it == s_format_items.end()) {
                    m_log_formatter_items.emplace_back(
    ...
    srvpro::ConfigVar<std::set<LogDefine> >::ptr g_log_defines = srvpro::Config::Lookup("logs", std::set<LogDefine>(),
                                                                                        "logs config");
