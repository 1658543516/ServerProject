...
srvpro::Logger::ptr g_logger1 = SRVPRO_LOG_NAME("system");

namespace srvpro {

    static srvpro::ConfigVar<int>::ptr g_tcp_connect_timeout = srvpro::Config::Lookup("tcp.connect.timeout", 5000,
                                                                                      "tcp connect timeout");
...
//
            g_tcp_connect_timeout->addListener([](const int &old_value, const int &new_value) {
                SRVPRO_LOG_INFO(g_logger1) << "tcp connect timeout changed from "
    ...
    do_io(int fd, OriginFun fun, const char *hook_fun_name, uint32_t event, int timeout_so, Args &&... args) {
        ...
        SRVPRO_LOG_DEBUG(g_logger1) << "do_io<" << hook_fun_name << ">";
        ...
        if (n == -1 && errno == EAGAIN) {
            ...
            if (rt) {
                SRVPRO_LOG_ERROR(g_logger1) << hook_fun_name << " addEvent("
                                           << fd << ", " << event << ")";
                if (timer) {
                    timer->cancel();
                }
...
#undef XX
    ...
    int connect_with_timeout(int fd, const struct sockaddr *addr, socklen_t addrlen, uint64_t timeout_ms) {
        ...
        if (timeout_ms != (uint64_t) -1) {
            timer = iom->addConditionTimer(timeout_ms, [winfo, fd, iom]() {
                ...
                t->cancelled = ETIMEDOUT;
                iom->cancelEvent(fd, srvpro::IOManager::WRITE);
            }, winfo);
        }

        int rt = iom->addEvent(fd, srvpro::IOManager::WRITE);
        if (rt == 0) {
            srvpro::Fiber::YieldToHold();
            if (timer) {
                timer->cancel();
            ...
            SRVPRO_LOG_ERROR(g_logger1) << "connect addEvent(" << fd << ", WRITE) error";
        }
    ...
    int connect(int sockfd, const struct sockaddr *addr, socklen_t addrlen) {
        return connect_with_timeout(sockfd, addr, addrlen, srvpro::s_connect_timeout);
    }
    ...
    ssize_t recvfrom(int sockfd, void *buf, size_t len, int flags, struct sockaddr *src_addr, socklen_t *addrlen) {
        return do_io(sockfd, recvfrom_f, "recvfrom", srvpro::IOManager::READ, SO_RCVTIMEO, buf, len, flags, src_addr,
                     addrlen);
    }
    ...
    int ioctl(int d, unsigned long int request, ...) {
        ...
        if (FIONBIO == request) {
            ...
            srvpro::FdCtx::ptr ctx = srvpro::FdMgr::GetInstance()->get(d);
            if (!ctx || ctx->isClose() || !ctx->isSocket()) {
    ...
    int setsockopt(int sockfd, int level, int optname, const void *optval, socklen_t optlen) {
        ...
        if (level == SOL_SOCKET) {
            if (optname == SO_RCVTIMEO || optname == SO_SNDTIMEO) {
                srvpro::FdCtx::ptr ctx = srvpro::FdMgr::GetInstance()->get(sockfd);
                if (ctx) {
